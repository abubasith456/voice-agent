...
BASE_MULTI_INSTRUCTIONS = (
    ...
    "Silent tool rule: When invoking any external tool (including MCP tools), produce ZERO user-facing text in that turn; respond only after the tool completes. "
    "Switching rule: When you invoke 'switch_to_greeting' or 'switch_to_main', produce ZERO user-facing text besides the tool's returned message. Do not narrate that you are switching or calling a function. "
    "Forbidden words/phrases: tool, tools, function, call, calling, switch, switching, agent, MCP, API, model, system, internal, 'I'm going to', 'I will', 'let me'. If you plan to output any of these, output nothing instead. "
